...
class Sharepoint {
    ...
    async executeGQL(url, opts) {
        ...
        if (!res.ok) {
            throw new Error(`Failed to execute ${url} with status ${res.status}`);
        }
        return res.json();
    }
    ...
    async getDriveRoot(accessToken) {
        ...
        try {
            ...
            headers.append('User-Agent', APP_USER_AGENT);
            headers.append('Accept', 'application/json');
            const spSite = this.appConfig.getSpSite();
            const response = await this.fetchWithRetry(`${spSite}/drive/root`, { headers });
    ...
    async getFilesData(filePaths, isGraybox) {
        const batchArray = [];
        ...
        for (let i = 0; i < batchArray.length; i += 1) {
            ...
            fileJsonResp.push(...await Promise.all(
                batchArray[i].map((file) => this.getFileData(file, isGraybox)),
            ));
     ...
     */
    async bulkCreateFolders(srcPathList, isGraybox) {
        const logger = getAioLogger();
    ...
    async copyFile(srcPath, destinationFolder, newName, isGraybox, isGrayboxLockedFile) {
        const logger = getAioLogger();
        const sp = await this.appConfig.getSpConfig();
    ...
    async saveFile(file, dest, isGraybox) {
        try {
            ...
            const sp = await this.appConfig.getSpConfig();
            let uploadFileStatus = await this.createSessionAndUploadFile(sp, file, dest, filename, isGraybox);
            ...
            if (uploadFileStatus.locked) {
                ...
                const copyFileStatus = await this.copyFile(newLockedFilePath, folder, filename, isGraybox, true);
                if (copyFileStatus) {
                    uploadFileStatus = await this.createSessionAndUploadFile(sp, file, dest, filename, isGraybox);
    ...
    async promoteCopy(srcPath, destinationFolder, fileName, sp) {
        ...
        const copyStatusInfo = await this.fetchWithRetry(`${sp.api.file.copy.gbBaseURI}${srcPath}:/copy?@microsoft.graph.conflictBehavior=replace`, options);
        const statusUrl = copyStatusInfo.headers.get('Location');
        ...
        if (!copySuccess) {
            ...
            await this.renameFile(spFileUrl, lockedFileNewName);
            const folder = this.getFolderFromPath(`${destinationFolder}/${fileName}`);
            const newLockedFilePath = `${folder}/${lockedFileNewName}`;
            const copyFileStatus = await this.copyFile(newLockedFilePath, folder, fileName, false, true);
            if (copyFileStatus) {
                copySuccess = await this.promoteCopy(srcPath, destinationFolder, fileName, sp);
                if (copySuccess) {
                    await this.deleteFile(sp, `${baseURI}${newLockedFilePath}`);
                }
            }
    ...
    async updateExcelTable(excelPath, tableName, values, isGraybox = false) {
        ...
        let itemId = '';
        if (isGraybox) {
            itemId = await this.getItemId(sp.api.file.get.gbBaseURI, excelPath);
        } else {
            itemId = await this.getItemId(sp.api.file.get.baseURI, excelPath);
        }
        if (itemId) {
            logger.info(`Updating table ${tableName} with values ${JSON.stringify({ values })}`);
