...
async function main(params) {
    ...
    // Update & Write the Batch Status to in progress "bulk-copy-batches/batch_status.json" file
    ...
    for (let i = 0; i < sourcePathObjs.length; i += 1) {
        ...
        try {
            ...
            // Determine the content path to fetch from
            let contentPath = mdPath || fragmentPath || sourceUrl;

            // If we have a sourcePath but no content path, construct the URL
            if (!contentPath && sourcePath) {
                // Convert sourcePath to URL format for fetching content
                // e.g., /homepage/drafts/sabya/dummy-bulk-copy-source-page.docx -> https://main--homepage--adobecom.aem.page/homepage/drafts/sabya/dummy-bulk-copy-source-page
            ...
            if (contentPath) { // If the file has a content path (mdPath, fragmentPath, sourceUrl, or constructed from sourcePath) then process the file
    ...
    if (processedFiles.length > 0) {
        ...
        // Ensure processedPathsJson is an object before proceeding
        ...
        // Combined existing If any processes already exist in processed_paths.json for the current batch
    ...
    if (failedProcesses.length > 0) {
        ...
        // Ensure processErrorsJson is an array before proceeding
        ...
        // Add new failed processes to the array
